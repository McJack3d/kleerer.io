...
`compare/` scores dietary supplements (whey, creatine, vitamin D3, magnesium, omega-3, multivitamins, zinc, vitamin C, collagen, probiotics, melatonin — and, shown apart, the botanicals ashwagandha, maca, rhodiola and curcumin) on a transparent, reproducible **0–100 Health & Compo Score (grades A–E)**, computed only from what's on the label and what a brand publicly proves — never from marketing claims:
...
- **Scope**: 193 coded products across 15 categories (11 nutrient categories + 4 botanicals), EU market with a French-market focus; nutrient prices snapshot **July 2026**, botanicals **September 2026**. All 193 are published; the review queue is empty. Anything withheld in future shows up in `compare/data.js` under `meta.n_withheld_for_review`.
